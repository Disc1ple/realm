...
provider "aws" {
  region = "us-east-1"
  access_key = var.auth[0].access_key
  secret_key = var.auth[1].secret_key
}
...
# #7. Create a network interface with an ip in the subnet that was creates in step 4
